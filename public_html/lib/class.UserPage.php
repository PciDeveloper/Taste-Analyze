...
CLASS getPage
{
	...
	function getPage($start, $limit, $page_scale=10, $total=0, $param="")
	{
		...
		if ($a_division[offset]>0){
			$prevStr=$this->inner_other(0,"<img src='/img/icon/notice_prev.png' alt='첫페이지'>", $option, "btn").$this->inner_other($a_division[from] - $this->limit,"<img src='/img/icon/notice_prev.png' alt='첫페이지'>", $option, "btn");
		}else{
			$prevStr="<a href='javascript:;' class=\"bt_prev\"><img src='/img/icon/notice_prev.png' alt='첫페이지'></a>";
		}

		if ($curBlock < $totBlock){
			$nextStr=$this->inner_other($a_division[from]+$contexture, "<img src='/img/icon/notice_next.png' alt='next'>", $option, "btn").$this->inner_other((ceil($this->allRow / $this->limit)-1)*$this->limit,"<img src='/img/icon/notice_next.png' alt='next'>", $option, "btn mg-none");
		}else{
			$nextStr="<a href='javascript:;' class=\"bt_next\"><img src='/img/icon/notice_next.png' alt='next'></a> ";
		}
		...
		for ($i=0;$i<$a_division[curSize];$i++)
		{
			...
			++$pos;
			$str.=($start==$this->start)? $this->inner_cur($pos) : $this->inner_other($start, $pos, $option, "num");
		}
		...
		if($this->allRow > 0)
			...
			$this->page_return = $prevStr."<a href='#' class='num_on'>1</a>".$nextStr;
	}

	function inner_cur($pos)
	{
		$this->curPage = $pos;
		return  "<a href='#' class='num_on'>".$pos."</a>";
	}
