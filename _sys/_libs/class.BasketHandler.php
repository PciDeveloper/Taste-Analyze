...
class BasketHandler extends MysqliHandler
{
	...
	public function __construct($is_direct=0, $mode="")
	{
			...
			if($v['gidx'])
			{
				...
				if($v['opt'])
				{
					...
					foreach($gopt as $vv)
					{
						...
						if($option)
						{
							$goods['stropt'][] = sprintf("<span class=\"option\">%s : %s %s</span>", $option['optnm'], $ex[1], (($ex[3] > 0) ? "(".$ex[2].number_format($ex[3]).")" : ""));
							if($ex[3] > 0)
							{
								if($ex[2] == "+")
									$optamt += ($ex[3] * $v['ea']);
	...
	*/
	...
	{
		$flag = false;
		if(_is_login() && !strcmp($this->mode, "basket"))
		{
			if(is_array($this->ar_item) && count($this->ar_item) > 0)
				$isqry = sprintf("insert into %s set userid='%s', goods='%s', regdt=now(), updt=now() ON DUPLICATE KEY UPDATE goods='%s', updt=now()", SW_BASKET, $_SESSION['SES_USERID'], base64_encode(serialize($this->ar_item)), base64_encode(serialize($this->ar_item)));
			else
				$isqry = sprintf("delete from %s where userid='%s'", SW_BASKET, $_SESSION['SES_USERID']);
	...
	*/
	public function _add_wishlist_item($gidx, array $item, array $gea)
	{
		...
		if($wish)
		{
			$isqry = sprintf("insert into %s set userid='%s', goods='%s', regdt=now(), updt=now() ON DUPLICATE KEY UPDATE goods='%s', updt=now()", SW_WISH, $_SESSION['SES_USERID'], base64_encode(serialize($wish)), base64_encode(serialize($wish)));
			if(parent::_execute($isqry))
				return true;
			else
				return false;
	...
	*/
	public function _get_wishlist()
	{
		if(_is_login())
		{
			$sqry = sprintf("select goods from %s where userid='%s'", SW_WISH, $_SESSION['SES_USERID']);
	...
	*/
	public function _mv_wish_basket($letter_no)
	{
		...
		else
		{
			...
			{
				$this->ar_item[] = array($sw['gidx'], $sw['opt'], $sw['ea']);
				if(self::_set_basket())
				{
					self::_del_wish_item($letter_no);
					gourl("/goods/basket.php", "P");
	...
	*/
	public function _del_wish_item($letter_no)
	{
		...
		if(count($re_wish) > 0)
			$isqry = sprintf("insert into %s set userid='%s', goods='%s', regdt=now(), updt=now() ON DUPLICATE KEY UPDATE goods='%s', updt=now()", SW_WISH, $_SESSION['SES_USERID'], base64_encode(serialize($re_wish)), base64_encode(serialize($re_wish)));
		else
			$isqry = sprintf("delete from %s where userid='%s'", SW_WISH, $_SESSION['SES_USERID']);
	...
	*/
	public function _del_wish_item_arr($item)
	{
		...
		if(count($rarr) > 0)
			$isqry = sprintf("insert into %s set userid='%s', goods='%s', regdt=now(), updt=now() ON DUPLICATE KEY UPDATE goods='%s', updt=now()", SW_WISH, $_SESSION['SES_USERID'], base64_encode(serialize($rarr)), base64_encode(serialize($rarr)));
		else
			$isqry = sprintf("delete from %s where userid='%s'", SW_WISH, $_SESSION['SES_USERID']);
	...
	*/
	public function _set_basket_tmp($type="sel", $data="")
	{
		...
		if(!strcmp($type, "all"))
		{
			foreach($this->item as $v)
			{
				// 재고 확인후 구매상품 담기 --- kkang(2019-12-26) //
				if($v['blimit'] != 3)
				{
					if($v['blimit'] == 1)
