...
class SaleHandler extends MysqliHandler
{
	...
	{
		$this->is_admin = (preg_match("/sadm/i", dirname($_SERVER['SCRIPT_NAME'])) && $_SESSION['SES_ADM_ID']) ? true : false;
		parent::__construct();
	}
		...
		while($row=mysqli_fetch_assoc($qry))
		{
			if($row['gopt'])
			{
				...
				foreach($gopt as $vv)
				{
					...
					if($option)
					{
						$row['stropt'][] = sprintf("<span class=\"size\">%s : %s %s</span>", $option['optnm'], $ex[1], (($ex[3] > 0) ? "(".$ex[2].number_format($ex[3]).")" : ""));
					}
				}
			}
	...
	*/
	public function _set_status_item($data)
	{
		if($this->vars['ordcode'])
		{
			if(!strcmp($data['act'], "receive"))
				$status = "202";
			else if(!strcmp($data['act'], "cancel"))
				$status = "300";
			else if(!strcmp($data['act'], "exchange"))
				$status = "400";
			else if(!strcmp($data['act'], "refund"))
				$status = "500";
			else if(!strcmp($data['act'], "ordok"))
				$status = "900";
