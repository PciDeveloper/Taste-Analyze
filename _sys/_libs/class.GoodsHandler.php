...
class GoodsHandler extends MysqliHandler
{
	...
	*/
	public function _get_goods_vars()
	{
		...
		if(!$row) msg("상품이 삭제되었거나 판매가 중단된 상품입니다.", -1, true);
		...
		if($row['related'])
		{
			...
			while($rrow = mysql_fetch_assoc($qry))
				$rgoods[] = $rrow;
	...
	*/
	private function _make_option_box()
	{
		...
		{
			$sbox = "";
			$sbox .= "<div class=\"goods_option\">";
			$sbox .= sprintf("<select name=\"gopt[]\" id=\"gopt_%d\" %s>", $j, (($this->opt[$i]['optreq']) ? "exp=\"".$this->opt[$i]['optnm']."을\"" : ""));
			$sbox .= sprintf("<option value=\"\"> :: %s 옵션선택 :: </option>", $this->opt[$i]['optnm']);
			if(!$this->opt[$i]['optreq'])
				$sbox .= sprintf("<option value=\"none\">%s 선택안함</option>", $this->opt[$i]['optnm']);
			for($o=0; $o < count($this->opt[$i]['optval']); $o++)
			{
				$sbox .= sprintf("<option value=\"%s∏‡%s∏‡%s∏‡%s\">%s%s</option>", $this->opt[$i]['optcd'], $this->opt[$i]['optval'][$o], $this->opt[$i]['optmark'][$o], $this->opt[$i]['optpay'][$o], $this->opt[$i]['optval'][$o], (($this->opt[$i]['optpay'][$o] > 0) ? "(".$this->opt[$i]['optmark'][$o]." ".number_format($this->opt[$i]['optpay'][$o])."원)" : ""));
			}
			$sbox .= "</select>";
			$sbox .= "</div>";
