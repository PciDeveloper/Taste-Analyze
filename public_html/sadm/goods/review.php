...
$sqry = sprintf("select a.*, b.name as gname, b.category as gcategory from %s a, %s b where a.gidx=b.idx %s", SW_REVIEW, SW_GOODS, $AddW);
...
<div class="row">
	 <div class="col-xs-12">
			...
			<tr>
				...
				<td>
					<select name="skey">
            <option value="a.name" <?=($skey == "a.name") ? "selected=\"selected\"" : "";?>>작성자</option>
            <option value="a.title" <?=($skey == "a.title") ? "selected=\"selected\"" : "";?>>제목</option>
            <option value="a.content" <?=($skey == "a.content") ? "selected=\"selected\"" : "";?>>내용</option>
...
<td>
    <button type="button" onclick="Common.winOpen('./win.review.php?idx=<?=$row['idx']?>', 'review', 'width=800, height=700, scrollbars=yes');" class="btn btn-white btn-info btn-xs">보기</button>
    <button type="button" onclick="Del('<?=$row['idx']?>');" class="btn btn-danger btn-white btn-xs">삭제</button>
</td>
</tr>
<?
	$letter_no--;
...
if($numrows < 1)
	echo("<tr><td colspan=\"9\" height=\"100\" align=\"center\">등록된 상품리뷰가 없습니다.</td></tr>");
