...
$sqry = sprintf("select * from %s where 1=1 and `gubun` = 'P' %s order by idx desc", SW_VISUAL, $AddW);
$db->_affected($numrows, $sqry);
$qry = $db->_execute($sqry);

while($row=mysql_fetch_array($qry))
{
	if(!strcmp($row['gubun'], 'P'))
		$arr_pc[] = $row;
	else
		$arr_mobile[] = $row;
}
...
<div class="row">
	...
	<p>
		<button type="button" onclick="Common.openLayerPopup('./ifrm.visual.php', 600, 400, '', 'no');" class="btn btn-white btn-info" >메인배너 등록</button>
	</p>
	...
	<tr onMouseOver="this.style.background='#eef3f7';" onMouseOut="this.style.background='';">
		...
		<td style="text-align:left;padding:3px 10px;">
			...
			<p class="user_tel mb5">타켓 : <?=($arr_pc[$i]['target'] == "_blank") ? "새창" : (($arr_pc[$i]['target']=="_parent") ? "현재창" : "링크없음")?></p>
			<p class="user_tel">링크 : <?=($arr_pc[$i]['url']) ? "<a href='".$arr_pc[$i]['url']."' target='_blank'>".$arr_pc[$i]['url']."</a>" : "링크없음";?>
		</td>
		<td><img src="/sadm/img/icon/icon_<?=($arr_pc[$i]['buse'] == "Y") ? "yes" : "no";?>.gif" /></td>
		<td> 
			<button type="button" onclick="Common.openLayerPopup('./ifrm.visual.php?idx=<?=$arr_pc[$i]['idx']?>', 600, 400, '', 'no');" class="btn btn-white btn-info btn-sm"/>수정</button> 
